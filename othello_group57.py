...
#Othello Assignment, Edmond Mentori & Aurora Shuti
...
class Graphics:
    ...
    def draw_possible(self, board, player):
        ...
        moves = allowed_moves(board, player).keys()
        
        for i, j in moves:
            self.canvas.create_oval(j*sq_w + left, i*sq_h + top, j*sq_w + right, i*sq_h + bottom, width=5, fill="blue", outline="blue")
...
class EvalAlphaBeta:
    def __init__(self, player, max_depth=6, max_time=5):
        self.player = player
        ...
        if len(posMovesMinPl) != 0:
            for move in posMovesMinPl:
                if self.overtimeFlag != 0:
                    return score
                elif time.time() - self.timer >= self.max_time:
...
def main():
    t = float(simpledialog.askstring("Choose Thinking Time", "Choose how long the computer can think on a move in seconds\nThe number should a nonzero number in seconds").strip().lower())
    ...
    if color == "b":
        ...
        player2 = EvalAlphaBeta(-1, max_time = t)
    elif color == "c":
        player1 = EvalAlphaBeta(1, max_time = t)
        player2 = EvalAlphaBeta(-1, max_time = t)
    else:
        player1 = EvalAlphaBeta(1, max_time = t)
    ...
    for _ in range(n_games):
        ...
        while True:
            ...
            if isinstance(player1, HumanPlayer):
                graphics.draw_possible(env.board, player1.player)
            player1.move(env)
            ...
            if isinstance(player2, HumanPlayer):
                graphics.draw_possible(env.board, player2.player)  
            player2.move(env)
            if not graphics.running: break
            graphics.draw(env.board)
            if env.game_over(): break
